...
public class PentominoShapes {
    ...
    public void Update() {
        ...
        //If there is a collision, add the pentomino to the grid and generate a new shape
        if (collision) { //This will only be activated if we can't do down and move the pentomino
            for (int x = 0; x < position.length; x++) {
                for (int y = 0; y < position[x].length; y++) {
                    //if the pentomino has !0 values, add 1 on the grid
                    if (position[x][y] != 0) {
                        grid.getGrid()[yPos + x][xPos + y] = 1;
                    }
                }
            }
        ...
        //Keep the pentomino inside the grid X sides
        if (MoveLegal("checkLeftorRight")) {
            xPos += deltaX; //+1 or -1 if pressed right or left
            collision = false;
        }

        //Keep the pentomino inside the grid Y sides
        if (MoveLegal("checkDown")) {
            ...
            System.out.println("--- COLISION BOTTOM ---");
            collision = true; //when the pentomino cannot be rotated nor moved
        }
