...
public class PentominoShapes {
    ...
    public void Update() {
        ...
        if ((xPos + deltaX +  position[0].length <= grid.getxAxis()) && (xPos + deltaX  >= 0)) { //using width_piece, because if you don't calculate it, "I" has another length value than "P"
            xPos += deltaX; //+1 or -1 if pressed right or left
        }

        //Keep the piece inside the grid Y sides
        if (yPos + position.length + 1<= grid.getyAxis()) {
    ...
    public void Rotate(){ //First we need to transpose the matrix(exchange rows and columns), Then reverse rows(Left rotation) or columns(Right rotations), we will rotate left, the rows
        ...
        if ((xPos + rotatedMatrix[0].length > grid.getxAxis()) || (yPos + rotatedMatrix.length > grid.getyAxis())) { //check if we rotate, that the piece stays in the grid
