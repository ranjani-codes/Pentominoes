...
public class Search
{
  ...
  public static UI ui = new UI(horizontalGridSize, verticalGridSize, 70);

  public static double countter = 0;
  ...
  public static void addPiece(int[][] field, int[][] piece, int pieceID, int x, int y)
  {
    ...
    // canPlace determines whether or not the piece overlaps on the field
    boolean canPlace = true;
    while(canPlace){

      for(int i = 0; i < piece.length; i++){
        for(int j = 0; j < piece[i].length; j++){
          if(piece[i][j] == 1 && canPlace){
            if(field[x+i][y+j] != -1){
              canPlace = false;
              break;
            }
  ...
  public static void main(String[] args)
  {
    ...
    System.out.println("Now, the magic is operating...");

    if (NumberOfPieces*5 == verticalGridSize*horizontalGridSize) {
      search();
    }
